...
router.get(
  "/attestation/options/:userId",
  jwtMiddlewareWebAuthn,
  async (req, res) => {
    //NOSONAR
    try {
...
router.post(
  "/attestation/result/:userId",
  jwtMiddlewareWebAuthn,
  async (req, res) => {
    //NOSONAR
    try {
      const verification = await webAuthnAPI.verifyRegistrationResponse(
        req.params.userId,
        req.body
      );
...
router.get("/assertion/options/:username", async (req, res) => {
  ...
  try {
    const _user: UserModel | undefined = (
      await User.find({ username: req.params.username }).select(
        "-password -__v"
      )
    )[0]; // NOSONAR
    ...
    const { options, forUserId } =
      await webAuthnAPI.generateAuthenticationOptions(_user);
