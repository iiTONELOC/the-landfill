...
router.post('/attestation/result/:userId', async (req, res) => {//NOSONAR
    try {
...
router.get('/assertion/options/:username', async (req, res) => {//NOSONAR
    try {
        const _user: UserModel | undefined = (await User.find({ username: req.params.username }).select('-password -__v'))[0]; // NOSONAR
...
router.post('/assertion/result/:userId', async (req, res) => {//NOSONAR
    try {
        const verification = await webAuthnAPI.verifyAuthenticationResponse(req.params.userId, req.body);
        return res.send(verification);
    } catch (error) {
        console.log(error);
