...
export const userProductMutations = {
  addUserProduct: async (
    _: any,
    {userId, barcode}: addUserProductMutationArgs,
    ...
    let existingProduct: ProductModel | null = await Product.findOne({barcode: {$in: [barcode]}})
      .exec()
      .catch((err: Error) => {
        throw new GraphQLError(err.message);
      });

    // if the barcode that was added was a book, then the barcode will include ISBN + the barcode
    // so we need to check for that as well
    if (!existingProduct && barcode.length >= 13) {
      existingProduct = await Product.findOne({barcode: {$in: [`ISBN ${barcode}`]}})
        .exec()
        .catch((err: Error) => {
          throw new GraphQLError(err.message);
        });
    ...
    if (existingProduct) {
      ...
      // create the product
      product = await Product.create({name, barcode: _barcode, source: source._id}).catch(
        (err: Error) => {
    ...
    const userProduct: UserProductModel | null = await UserProduct.create({
      userId,
      barcode,
  ...
  updateUserProduct: async (
    _: any,
    {userProductId, productAlias}: editUserProductMutationArgs,
    {user}: AuthenticatedContext,
  ) => {
    await authenticatedUser(user as UserModel);
