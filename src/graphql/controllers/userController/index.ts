...
export const userMutations = {
  ...
  loginUserDevice: async function loginUserDevice(
    _: any,
    {
      ...
      password,
      deviceKey,
    }: { username: string; password: string; deviceKey: string }
  ) {
    try {
  ...
  loginUserApp: async function loginUserApp(
    _: any,
    {
      ...
      password,
      appKey,
    }: { username: string; password: string; appKey: string }
  ) {
    try {
      const user = await User.findOne({ username }); //NOSONAR
      if (!user) {
      ...
      // look up an app key for the user - only one key is registered at a time
      const userAppKey = await AppKey.findOne({ userId: user?._id }); //NOSONAR

      const validAppKey = userAppKey?.isCorrectKey(appKey);
      // check if the password is valid
      const validPassword = await user.isCorrectPassword(password);

      if (!validPassword || !validAppKey) {
        throw new GraphQLError(INCORRECT_CREDENTIALS);
      }
  ...
  deleteUser: async function deleteUser(
    ...
    try {
      // delete the user
      const deleted: UserModel = (await User.findByIdAndDelete(
        user as UserModel
      ).populate("lists")) as UserModel;
  ...
  generateAppKey: async function generateAppKey(
    _: any,
    __: any,
    { user }: AuthenticatedContext
  ) {
